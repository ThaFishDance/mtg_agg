...
function extractCardCandidate(transcript) {
...
export default function useVoiceCardRecognition({ enabled }) {
  ...
  const lastLookupRef = useRef({ name: '', at: 0 })
  ...
  useEffect(() => {
    ...
    async function lookupCard(candidate, transcript) {
    ...
    function scheduleLookup(transcript, immediate = false) {
